...
class ParentKidComponent extends Component {
    ...
    renderChallenges = ()=>{
        ...
        return(
            this.props.user.parent.kids.filter(kid=> kid._id === this.props.user.selectedKid._id)[0].categories
            .map(category=>{
                const totalAnswered = category.questions.filter(question=> question.answered === true).length
                ...
                return (
                    <TouchableOpacity key={category.name} onPress={() => alert(JSON.stringify(category.questions))}>
                        <View style={styles.challengeContainer}>
                            <Text style={styles.challengeText}>{category.name}</Text>
                            <View style={{ backgroundColor: '#f5dc82', width: 'auto', height: 20, borderRadius: 50 }}>
                                <View style={{ backgroundColor: '#ed553b', width: percentComplete, height: 20, borderRadius: 50 }}>
                                </View>
                            </View>
                            <Text style={styles.challengeSubText}>{`${totalAnswered}/${totalQuestions} COMPLETE`}</Text>
                        </View>
    ...
    calculatePercent = () => {
        ...
        this.props.user.parent.kids.filter(kid => kid._id === this.props.user.selectedKid._id)[0].categories
            .map(category => {
                // console.log(`${JSON.stringify(category.questions.length)}*********************************************`)
                    ...
                    <View style={styles.challengeColumn}>
                        <View style={styles.centered}>
                            ...
                            <View style={styles.challengeScrollView}>
                                ...
                                >
                                    {/* <TouchableOpacity onPress={()=>alert(JSON.stringify(this.props.kid.categories))}>
                                    <Text>TEST</Text>
                                    </TouchableOpacity> */}
                        ...
                        >
                            <ScrollView>
                                <View style={styles.modalView}>
                                    ...
                                    <Text style={styles.modalTitle}>ARE YOU SURE YOU WANT TO DELETE THIS PROFILE? </Text>
...
const styles = StyleSheet.create({
    ...
    title:{
        fontFamily: 'Dosis',color:'#ed553b',fontSize:24  
    },
    text:{
        fontFamily: 'Dosis',color:'#ed553b',fontSize:20  
    },
    fieldBackground:{
        width:'80%',
        marginTop:15,
