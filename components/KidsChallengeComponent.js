...
import {TouchableOpacity,Text,TextInput,View,FlatList, StyleSheet, KeyboardAvoidingView, ScrollView, SafeAreaView, Image, ActivityIndicator, Alert} from 'react-native';
...
class LogInComponent extends Component {
    ...
    RenderImage = (image) => {
        if (image === '../assets/HistoryIcon.png') {
            ...
            return (
                <Image style={styles.image} source={MathIcon} />
            )
        } else if (image === '../assets/ScienceIcon.png') {
            return (
                <Image style={styles.image} source={ScienceIcon} />
            )
        } else if (image === '../assets/ReadingIcon.png') {
            return (
                <Image style={styles.image} source={ReadingIcon} />
            )
        }else{
            return(<View></View>)
        }
    ...
    renderSmallChallenge = (category) => {
        const answered = category.questions.filter(question=> question.answered === true).length
        const questionTotal= category.questions.length
        return (
            <View key={category._id} style={styles.categoryRow}>
                <TouchableOpacity style={styles.categoryItem} onPress={() => this.props.navigation.navigate('Questions', { category: category })} >
                    <View style={styles.categoryView}>
                        <View>
                            <Text style={styles.categoryText}>{category.name}</Text>
                            {answered === questionTotal ? <Text>Check mark</Text> : <Text style={styles.challengeSubText}>{`${answered}/${questionTotal} \nQUESTIONS\nLEFT`}</Text>}
                        </View>
                        <View>
    ...
    renderChallenges =()=>{
        if (this.props.user.selectedKid.categories.length > 0) {
            //Sort categories
            const categories = this.props.user.selectedKid.categories.sort((a, b) => (a.lastAccessed < b.lastAccessed) ? 1 : -1)
            //categories.sort((a, b) => (a.lastAccessed > b.lastAccessed) ? 1 : -1)
            const lastAccessed = categories[0]
            const totalAnswered = lastAccessed.questions.filter(question=> question.answered === true).length

            const notLast = categories.map((category)=>{
                
                if(category !=lastAccessed){
                    return(
                        this.renderSmallChallenge(category)
                    )
                }
            })
                    ...
                    <View key={category._id} style={styles.categoryRow}>
                        <TouchableOpacity style={styles.categoryItem} onPress={() => this.props.navigation.navigate('Questions', { category: category })} >
                            <View style={styles.categoryView}>
                                ...
                                <View>
                                    {answered === questionTotal ? <Text style={styles.challengeSubTextGreen}>COMPLETED</Text> : <Text style={styles.challengeSubText}>{`${answered}/${questionTotal} \nQUESTIONS\nLEFT`}</Text>}
                                </View>
                            </View>
                        </TouchableOpacity>
                    </View>
            ...
            return (
                ...
                >
                    <TouchableOpacity style={styles.lastAccessed} onPress={()=>{this.props.navigation.navigate('Questions',{category:lastAccessed})}}>
                        ...
                        <View style={styles.lastAccessedImageView}>
                            <View style={styles.center}>
                                ...
                                {totalAnswered !== lastAccessed.questions.length? 
                                    <Text style={styles.challengeSubText}>{`${totalAnswered}/${lastAccessed.questions.length} QUESTIONS LEFT`}</Text> :
